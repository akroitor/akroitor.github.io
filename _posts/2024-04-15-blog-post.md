...
$$
\begin{align*}
\left| \zeta(3) - \frac{2 (\text{lcm} \set{ 1, \cdots, n})^3 a_n}{ 2 (\text{lcm} \set{ 1, \cdots, n})^3 b_n} \right|
\end{align*}
$$

shrinks like \\( ((1+ \sqrt{2})^8)^{-n} \\) while \\(1 /2 (\text{lcm} \set{ 1, \cdots, n})^3 b_n \\) shrinks like \\( ( e^3(1+ \sqrt{2})^4 )^{-n} \\). Thus we have finally constructed a sequence that satisfies the conditions of our lemma. It follows that \\( \zeta(3) \\) is irrational.
